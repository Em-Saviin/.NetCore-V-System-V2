...
namespace V_System_Core.Component
{
    public static class MyHelperSql
    {
       public static   object defaultSelect2 = new
        ...
        public static (int code, string message) ExecuteInsert(DbContext dbContext, string sqlQuery, params SqlParameter[] parameters)
        {
            try
            {
                using (var command = dbContext.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        return (0, "Insert successful"); // 0 for success
                    }
                    else
                    {
